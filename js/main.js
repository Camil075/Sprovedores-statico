...
function parcearCSV(csvText) {
  ...
  const headers = lienas[0].split(",").map(h => h.trim().toLowerCase());
  if (!headers.includes('nombre') && !headers.includes('categoria')) {
    return [];
  }
  ...
  if (!lista || lista.length === 0) {
    grid.innerHTML = `
      <div style="grid-column: 1 / -1; text-align: center; padding: 3rem; color: #666;">
        <p style="font-size: 1.2rem; font-weight: 500;">No se encontraron productos disponibles.</p>
      </div>
    `;
    return;
  }
